  ...
  async function createOrder(payload) {
    try {
      ...
      const insertedOrder = await knex.transaction(async (trx) => {
        ...
        const [orderId] = await trx("orders").insert(order).returning("*"); // Trả về tất cả các cột của đơn hàng đã được insert
        ...
        // Trả về đơn hàng đã được insert cùng với thông tin chi tiết
        const insertedOrder = await trx("orders").where("id", orderId).first();

        return insertedOrder;
      });

      return { message: "Order created successfully!", order: insertedOrder };
    } catch (error) {
